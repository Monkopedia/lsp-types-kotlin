...
public class LspServerProcess internal constructor(
     ...
     * Closing the child's stdin/stdout ends ksrpc's posix reader/writer threads
     * (EOF -> thread exit), so this terminates deterministically. Idempotent.
     */
    public fun close() {
        kill()
        close(stdinWriteFd)
...
public suspend fun spawnLspServer(
    ...
    when {
        ...
        pid == 0 -> {
            // CHILD. Between fork and execvp only async-signal-safe calls are made
            // (dup2/close/execvp) — no Kotlin heap allocation that touches the K/N
            // runtime — so this is safe even though the parent runtime is threaded.
...
/** Build the NULL-terminated argv and `execvp`. Only returns on failure. */
private fun execChild(command: List<String>) {
    memScoped {
        val argv = command.map { it.cstr.getPointer(this) } + listOf(null)
        execvp(command.first(), argv.toCValues().ptr)
    }
}
