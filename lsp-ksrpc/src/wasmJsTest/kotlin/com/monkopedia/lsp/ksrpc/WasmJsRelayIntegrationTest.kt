...
/**
 * wasmJs (Node-hosted) relay integration round-trip (issue #48, epic #41).
 *
 * Validates kodemirror's actual consumption model: a [ConformanceLanguageServer]
 * served over an in-process ksrpc relay channel (paired ktor [ByteChannel]s, the
 * same shape as the JVM in-memory relay case) and consumed as a generated
 * [KsrpcLanguageServer] stub — all running on the wasmJs target. This proves the
 * relay + generated `@KsService` dispatch + typed-union serialization work on
 * wasmJs WITHOUT any external WebSocket / process infrastructure (web can't spawn
 * processes; it consumes a `LanguageServer` relayed over a ksrpc channel).
 *
 * The branch-selection contract is the same one [ConformanceLanguageServer]
 * documents and the JVM `ConformanceFixtureSmokeTest` asserts; this is the wasmJs
 * mirror of that executable contract.
 *
 * ## No-hang discipline
 *
 * wasmJs runs on Node's single-threaded event loop. Both the relay server's
 * receive loop and the client stub share that loop, so:
 * - every wire call is wrapped in [withTimeout] (a stuck call fails fast rather
 *   than hanging the Node test runner),
 * - teardown is deterministic channel-close + job-cancel (no blocking await on
 *   the server loop, which is the ksrpc #201-class hang risk on JS/native
 *   teardown).
 */
class WasmJsRelayIntegrationTest {

    @Test
    fun `every documented union branch round-trips over the wasmJs relay`() = runTest {
        withRelayConnection { remote ->
            withTimeout(TIMEOUT_MS) {
                ...
                // shutdown / exit must complete cleanly over the relay.
    ...
    private suspend fun assertHoverBranches(remote: KsrpcLanguageServer) {
        ...
        assertNotNull(markup, "line 0 hover should return a result")
        assertTrue(
        ...
        assertNotNull(marked, "line 1 hover should return a result")
        assertTrue(
        ...
        assertNotNull(markedArray, "line 2 hover should return a result")
        assertTrue(
    ...
    private suspend fun assertDefinitionBranches(remote: KsrpcLanguageServer) {
        fun params(line: Int) = DefinitionParams(
            textDocument = TextDocumentIdentifier(ConformanceLanguageServer.Uri.MAIN),
            position = pos(line)
        )
        val single = remote.textDocumentDefinition(params(ConformanceLanguageServer.Lines.SINGLE))
        val singleValue = (single as TextDocumentDefinitionResult.DefinitionValue).value
        assertTrue(singleValue is SingleOrArray.Single, "line 0 definition should be single")

        val array = remote.textDocumentDefinition(params(ConformanceLanguageServer.Lines.ARRAY))
        ...
        val link = remote.textDocumentDefinition(params(ConformanceLanguageServer.Lines.LINK))
        assertTrue(
    ...
    private suspend fun assertTypeDefinitionBranches(remote: KsrpcLanguageServer) {
        ...
        val single =
            remote.textDocumentTypeDefinition(params(ConformanceLanguageServer.Lines.SINGLE))
        ...
        val array = remote.textDocumentTypeDefinition(params(ConformanceLanguageServer.Lines.ARRAY))
    ...
    private suspend fun assertImplementationBranches(remote: KsrpcLanguageServer) {
        ...
        val single =
            remote.textDocumentImplementation(params(ConformanceLanguageServer.Lines.SINGLE))
        ...
        val array = remote.textDocumentImplementation(params(ConformanceLanguageServer.Lines.ARRAY))
    ...
    private suspend fun assertReferences(remote: KsrpcLanguageServer) {
        ...
        assertNotNull(refs, "references should return a result")
        assertEquals(3, refs.size)
    }

    /**
     * Run [block] against a remote [KsrpcLanguageServer] stub backed by a fresh
     * [ConformanceLanguageServer] served over an in-process ksrpc relay. The
     * relay server's receive loop runs as a child job of the test scope; teardown
     * closes both channels and cancels that job (deterministic, no blocking
     * await) so the wasmJs/Node test runner never hangs on teardown.
     */
    private suspend fun withRelayConnection(block: suspend (KsrpcLanguageServer) -> Unit) =
        // Run on the real (event-loop) dispatcher rather than runTest's virtual
        // time, so the ByteChannel I/O actually progresses and the [withTimeout]
        // guards measure wall-clock time. This mirrors the JVM smoke test, which
        // wraps the relay body in withContext(Dispatchers.Default).
