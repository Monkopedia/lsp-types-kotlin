...
class UnionGenerator(private val resolver: TypeResolver) {
     ...
     * with a REAL structure of the same name — LSP 3.18 promoted several previously
     * anonymous inline literals (`TextDocumentFilterLanguage`,
     * `NotebookDocumentFilterScheme`, …) into named top-level structures, and the
     * spec authors picked the same names this generator synthesizes.
     *
     * When that happens the structure is already emitted from `model.structures`, so
     * synthesizing a second class for it is a redeclaration. Register the interface
     * on the existing structure instead and skip the synthetic copy.
     */
